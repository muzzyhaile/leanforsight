...
export const generateScenarios = async (topic: string): Promise<Scenario[]> => {
  ...
  const schema: Schema = {
    ...
    items: {
      type: Type.OBJECT,
      properties: {
        type: { type: Type.STRING, enum: [ScenarioType.BEST_CASE, ScenarioType.WORST_CASE, ScenarioType.PREFERRED, ScenarioType.PLAUSIBLE] },
...
export const generateStrategy = async (
  ...
  const modelId = 'gemini-2.5-flash'; // Flash is sufficient for this
